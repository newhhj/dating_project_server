...
namespace Strawberry.Server.Manager.Pages.LoveFortune
{
    ...
    public partial class LoveFortuneList : IDisposable
    {
        ...
        private Database.Tables.LoveFortune[] Items { get; set; }
        ...
        private void GetPageData()
        {
            ...
            var query = this.Db.LoveFortunes
                .AsQueryable();
        ...
        {
            this.Js.InvokeVoidAsync("open", $"/lovefortunedetail", "lovefortunedetail", "left=0,top=0,width=500,height=720");
        }

        private void OpenDetail(Database.Tables.LoveFortune item)
        {
            this.Js.InvokeVoidAsync("open", $"/lovefortunedetail/{item.Id}", "lovefortunedetail", "left=0,top=0,width=500,height=720");
        }

        private async Task AcceptItemAsync(Database.Tables.LoveFortune item)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(item.Message))
                    throw new Exception("문구 내용을 입력하세요.");
        ...
        private async Task RemoveItemAsync(Database.Tables.LoveFortune item)
        {
            if (await this.Js.InvokeAsync<bool>("confirm", "해당 항목을 삭제하시겠습니까?"))
            {
                this.Db.LoveFortunes.Remove(item);
                this.Db.SaveChanges();
